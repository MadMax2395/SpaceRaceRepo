...
class ItemToRecover: SKSpriteNode, GameSprite {
    
    var textureAtlas: SKTextureAtlas = SKTextureAtlas(named: "bonus.atlas")
    var selfAnimation = SKAction()
    var typeItemToRecover:TypeItemToRecover = .unkown
    var hitAnimation = SKAction()
    
    func setTypeItemToRecover(type:TypeItemToRecover){
        self.typeItemToRecover = type
    }
    ...
    func setStaticImageAndSetPhysicsBody(name: String) {
        ...
        self.physicsBody?.categoryBitMask = PhysicsCategory.itemToRecover.rawValue
